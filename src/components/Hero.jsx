export function Hero(){
    return(
        <>
            <div style={{width: '100%', height: '100%', paddingTop: 208, paddingBottom: 476, paddingLeft: 71, paddingRight: 71, background: 'white', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', display: 'inline-flex'}}>
            <img style={{width: 284, height: 267}} src="./src/assets/naval2.jpg" />
            <input type="text" style={{width: 1298, height: 73, background: 'rgba(65, 65, 65, 0.39)', boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25) inset', borderRadius: 35}} />
            </div>
        </>
    )
}